  ...
  filtered.forEach((order) => {
    ...
    article.innerHTML = `
      <div class="order-card__top">
        <div class="order-number"><strong>Pedido #${order.id}</strong><small>${escapeHtml(createdAt)}</small></div>
      ...
      <div class="order-customer">
        ...
        <span>${escapeHtml(order.delivery_method)}${order.address ? ` · ${escapeHtml(order.address)}` : ""}</span>
        <span>Pagamento: ${escapeHtml(order.payment_method)}</span>
      </div>
      <div class="order-items">${items}</div>
      <div class="order-total"><span>Total</span><strong>${money.format(order.total || 0)}</strong></div>
      ${order.notes ? `<p class="order-notes"><strong>Obs.:</strong> ${escapeHtml(order.notes)}</p>` : ""}
      <div class="order-actions">
        <select data-status-select="${order.id}" aria-label="Status do pedido ${order.id}">${options}</select>
        <button class="button button--primary" type="button" data-save-status="${order.id}">Salvar</button>
      </div>`;
...
els.ordersList.addEventListener("click", (event) => {
  ...
  if (!button) return;
  updateStatus(Number(button.dataset.saveStatus));
});
